...
import TowingCard from 'src/components/towing/TowingCard';
import UserCard from 'src/components/user/UserCard';
import { useCalendarEventActions } from 'src/hooks/useCalendarEvents';
import useDisclosure from 'src/hooks/useDisclosure';
import type { CalendarEvent, Towing, User } from 'src/types';
...
const Calendar: FC<Props> = ({
	...
	const calendarMethods = useMemo(
		() => ({
			...
			selectRange: (arg: DateSelectArg) => {
				...
				if (editable) onRangeSelect?.({ start: arg.start, end: arg.end });
			},

			selectEvent: (arg: EventClickArg) => {
				if (editable) onEventSelect?.(arg.event);
			},

			resizeEvent: async (arg: EventResizeDoneArg) => {
				if (editable) onEventResize?.(arg.event);
			},

			dropEvent: async (arg: EventDropArg) => {
				if (editable) onEventDrop?.(arg.event);
			},
	...
	useEffect(() => {
		const handleKeyDown = (e: KeyboardEvent) => {
			...
			// Delete the event
			else if (e.ctrlKey && e.key === 'x' && hoveredEvent) {
				e.preventDefault();
				calendarEventActions.delete(hoveredEvent.id).catch((error) => console.error(error));
			}
		};
...
const ListEventContent = (eventInfo: EventContentArg) => {
	const user = eventInfo.event.extendedProps.userAssignee as User | undefined;
	const towing = eventInfo.event.extendedProps.towingAssignee as Towing | undefined;
	const userDisclosure = useDisclosure();
	const towingDisclosure = useDisclosure();

	const isOpen = userDisclosure.isOpen || towingDisclosure.isOpen;

	const handleClose = useCallback(() => {
		userDisclosure.close();
		towingDisclosure.close();
	}, []);
		...
		<>
			...
			>
				<Stack direction="row">
					...
					<Stack ml={eventInfo.timeText ? 1 : undefined}>
						...
						{/* subtitle details */}
						{towing && <Typography variant="caption">Remorque: {towing.name}</Typography>}
						{user && <Typography variant="caption">Téléphone: {user.phone}</Typography>}
					</Stack>
				</Stack>
				...
				<Stack direction="row" spacing={1.5}>
					...
					{towing && (
						<RowAction
							...
							Icon={Icons.towing}
							onClick={towingDisclosure.toggle}
						/>
					)}
				</Stack>
			</Stack>

			{/* User or Towing Details Dialog */}
			<Dialog
				fullWidth
				open={isOpen}
				maxWidth="sm"
			...
			>
				{userDisclosure.isOpen
					? user && <UserCard data={user} title="Utilisateur" />
					: towing && <TowingCard data={towing} />}
			</Dialog>
		</>
	);
};
